...
class _BranchScreenState extends State<BranchScreen> {
      ...
      child: Container(
        ...
        child: Stack(
          children: [
            ...
            Positioned(top: 200, left: 0, right: 0, child: Container(height: 2, color: Colors.black12)),
                  ...
                  children: [
                    if (isSelected)
                      Container(
                        ...
                        child: Text(b['name'].split(" ").last, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold)),
                      ),
                    Icon(Icons.location_on, color: isSelected ? Colors.orange : const Color(0xFF000DC0), size: isSelected ? 40 : 30),
                  ],
                ),
              );
            }),
              ...
              Padding(
                ...
                child: Container(
                  ...
                  child: TextField(
                    ...
                    decoration: const InputDecoration(border: InputBorder.none, icon: Icon(Icons.search, size: 20), hintText: "Tìm chi nhánh..."),
                  ),
                ),
              ),
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return GestureDetector(
                      onTap: () => setState(() => selectedBranchId = isSelected ? null : b['id']),
                      child: AnimatedContainer(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(color: isSelected ? const Color(0xFF000DC0).withOpacity(0.3) : Colors.grey[200]!),
                        ),
                        child: Column(
                          children: [
                            Row(
                              ...
                              children: [
                                CircleAvatar(backgroundColor: const Color(0xFF000DC0).withOpacity(0.1), child: const Icon(Icons.location_on, color: Color(0xFF000DC0), size: 20)),
                                const SizedBox(width: 12),
                                Expanded(
                                  child: Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(b['name'], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)), Text(b['distance'], style: const TextStyle(color: Colors.grey, fontSize: 11))]),
                                      const SizedBox(height: 4),
                                      Text(b['address'], style: const TextStyle(color: Colors.grey, fontSize: 12)),
                                      const SizedBox(height: 8),
                                      Row(children: [
                                        Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: b['isOpen'] ? Colors.green[50] : Colors.red[50], borderRadius: BorderRadius.circular(10)), child: Text(b['isOpen'] ? "Đang mở cửa" : "Đã đóng cửa", style: TextStyle(color: b['isOpen'] ? Colors.green : Colors.red, fontSize: 10, fontWeight: FontWeight.bold))),
                                        const SizedBox(width: 10),
                                        Text(b['hours'], style: const TextStyle(color: Colors.grey, fontSize: 11)),
                                      ]),
                                    ],
                                  ),
                                ),
                            ...
                            if (isSelected) ...[
                              ...
                              Row(children: [const Icon(Icons.phone, size: 14, color: Color(0xFF000DC0)), const SizedBox(width: 8), Text(b['phone'], style: const TextStyle(fontSize: 12))]),
                              const SizedBox(height: 15),
                              Row(children: [
                                Expanded(child: ElevatedButton.icon(onPressed: () {}, icon: const Icon(Icons.navigation, size: 16), label: const Text("Chỉ đường"), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF000DC0), foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))))),
                                const SizedBox(width: 10),
                                OutlinedButton.icon(onPressed: () {}, icon: const Icon(Icons.phone, size: 16), label: const Text("Gọi"), style: OutlinedButton.styleFrom(foregroundColor: const Color(0xFF000DC0), side: const BorderSide(color: Color(0xFF000DC0)), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)))),
                              ]),
                            ],
                          ],
                        ),
  ...
  Widget _buildPillBottomNav() {
    return Positioned(
      ...
      child: Container(
        height: 60,
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(30), boxShadow: const [BoxShadow(color: Colors.black12, blurRadius: 15, offset: Offset(0, 5))]),
        child: Row(
  ...
  Widget _pillNavItem(IconData icon, String label, int index) {
    ...
    return GestureDetector(
      onTap: () {
        if (index == 0) {
          // Bấm Trang chủ: Thoát sạch để về Home
          Navigator.of(context).popUntil((route) => route.isFirst);
        } else if (index == 1) {
          // Bấm Tìm kiếm: Thoát trang Chi nhánh để về lại trang Tìm kiếm
